...
const MessageTypeHandler = {
  message: function message(payload, socket) {
    ...
    /*
    * recipient variable can be a actual socket connection, or if recipent is not online/ not registered via calling setIdentifier, 
    * the variable will be an MissedMessageHandler object which implements a send interface.
    */
    ...
    if (recipient.isOffline) {
      ...
      const info_message = JSON.stringify({ type: 'info', from: 'server', message: `${to} is not online at the moment. Message will be delivered to ${to} as soon as he/she comes back online.`})
